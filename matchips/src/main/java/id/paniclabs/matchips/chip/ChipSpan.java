...
import id.paniclabs.matchips.R;
...
public class ChipSpan extends ImageSpan implements Chip {
    ...
    public ChipSpan(@NonNull Context context, @NonNull CharSequence text, @Nullable Drawable icon, Object data) {
        ...
        mTextColor = ContextCompat.getColor(context, R.color.chip_white_text_color);
        mIconBackgroundColor = ContextCompat.getColor(context, R.color.chip_material_background);
    ...
    private void drawText(Canvas canvas, float x, int top, int bottom, Paint paint, CharSequence text) {
        ...
        float adjustedY = top + ((height >> 1) + ((-fm.top - fm.bottom) / 2));
    ...
    private void drawIconBitmap(Canvas canvas, float x, int top, int bottom, Paint paint) {
        ...
        float xInsetWithinCircle = (height - bitmapCanvas.getWidth()) >> 1;
        ...
        float yInsetWithinCircle = (height - bitmapCanvas.getHeight()) >> 1;
